...
#Transform the type of Genres from chr to num
edx$genres<-as.double(edx$genres)
...
#Shows that movilens, originally has genres and titles as strings and for analises 
...
#A summary to hace a overview of edx
summary(edx)
...
#the trainig test used to develop this project.
...
#Saving plot to be used in the report. Will be loaded into the RMD
#file. 
jpeg(file="figures/figure1x.jpeg")
figure1x
dev.off()
...
#Saving plot to be used in the report. Will be loaded into the RMD
#file. 
jpeg(file="figures/figure2.jpeg")
figure2
dev.off()
...
#Saving plot to be used in the report. Will be loaded into the RMD
#file. 
jpeg(file="figures/figure3.jpeg")
figure3
dev.off()

#And how is the rating distribution among the  movies?
most_rated_2 <- edx %>%
  group_by(movieId) %>%
  summarize(count=mean(rating)) %>%
  arrange(desc(count))

figure4<-most_rated_2 %>%
  ggplot(aes(count)) +
  geom_histogram(binwidth = 0.5, fill = "red", col = "white") +
...
#And these are the ten less rated movies
less_rated <- edx %>%
  group_by(movieId) %>%
  summarize(count=n()) %>%
  top_n(-10,count) %>%
...
less_rated<-inner_join(less_rated,movieID_title,by="movieId")
head(less_rated)
...
figure7<-year_distribuition %>% 
  ...
  labs(title="Ratings per Year Title")
...
rm(figure1,figure1x,figure2,figure3,figure4,figure5,figure6,figure7)
rm(less_rated, most_genre, most_genre_2,most_rated,most_rated_2, userIdList)
...
RMSE_Table<-RMSE_Table %>% add_row(Method="Year Bias", RMSE=year_bias)
...
RMSE_Table<-RMSE_Table %>% add_row(Method="Final Result", RMSE=final_result)
RMSE_Table
